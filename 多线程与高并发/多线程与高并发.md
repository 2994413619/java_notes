...
- 多个codition本质就是多个等待队列，可以notifyAll()指定的线程组
...
ReentrantLock源码（类的关系图、方法调用图）：

缺图：五——1  06:50



(缺图，自己画)继承关系：NonfairSync ——> Sync ——> AQS（class名:AbstractQueuedSynchronizer）

缺图：调用关系 五——1 10:48



Template method：AQS.acquiree(1)调用了tryAcquire(1)，AQS自己有改方法，但是实际运行中调用的是子类的tryAcquire(1)，子类重写该方法
